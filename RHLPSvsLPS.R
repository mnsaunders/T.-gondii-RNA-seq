...
data<-RHLPSvsLPS %>% remove_rownames %>% column_to_rownames(var="Geneid")
Counts<-filter(data, "LPS_1" >0, "LPS_2" >0,"LPS_3" >0, "LPS_4" >0)

condition <- factor(c("LPS","LPS","LPS","LPS","RHLPS","RHLPS","RHLPS","RHLPS"))
...
plotDispEsts(deseq)
res<- results(deseq, contrast=c("condition", "RHLPS", "LPS"))
res
nona<-na.omit(res)
nona
...
write.csv(nona, "LPSplusinfections.csv")
LPSplusinfections1<-filter(LPSplusinfections, padj<0.05)
names(LPSplusinfections1)[names(LPSplusinfections1) == '...1'] <- 'gene_id'



comdataup<-subset(res, padj<1e-8)
comdataup<-subset(comdataup, log2FoldChange>1.8)
comdataup<-as.data.frame(comdataup)
comdataup<-rownames_to_column(comdataup)

comdatadown<-subset(res, padj<1e-8)
...
ggplot(genedat, aes(x=log2FoldChange, y=-log10(pvalue), col=diffexp))+
  ...
  labs(x="log2FoldChange", y="-log(pvalue)", title= "T. gondii Drives Regulation in Dual Infected Neutrophils")
