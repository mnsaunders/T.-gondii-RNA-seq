...
data<-PCA_aligns %>% remove_rownames %>% column_to_rownames(var="...1")
Counts<-filter(data, "RH_1" >0, "RH_2" >0,"RH_3" >0, "RH_4" >0)

condition <- factor(c("LPS","LPS","LPS","LPS","RH","RH","RH","RH"))
...
nona$diffexp[nona$log2FoldChange>0 & nona$pvalue<0.05] <-'up'
nona$diffexp[nona$log2FoldChange<0 & nona$pvalue<0.05] <-'down'
write.csv(nona, "generegulation.csv")
generegulation1<-filter(generegulation, padj<0.05)
names(generegulation1)[names(generegulation1) == '...1'] <- 'Geneid'

genedat<-generegulation1 %>%
  left_join(RHannotated, by = "Geneid")
...
ggplot(genedat, aes(x=log2FoldChange, y=-log10(pvalue), col=diffexp))+
  ...
  labs(x="Gene Regulation", title="RH vs RHLPS parasite")

up<-filter(genedat, log2FoldChange> 5.5)
down<-filter(genedat, log2FoldChange< -1.7)
top<-rbind(up,down)
...
ggplot(top, aes(y=log2FoldChange, x=gene, fill=diffexp))+
  ...
  labs(y="Gene Regulation", title="RH vs RHLPS parasite")
